...
RUN turbo prune monitoring-service --docker
...
COPY --from=installer --chown=nodeuser:nodejs /app/apps/monitoring-service/dist ./dist
COPY --from=installer --chown=nodeuser:nodejs /app/apps/monitoring-service/package.json .
