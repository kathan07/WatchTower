...
COPY --from=installer --chown=nodeuser:nodejs /app/apps/alerting-service/dist ./dist
COPY --from=installer --chown=nodeuser:nodejs /app/apps/alerting-service/package.json .
