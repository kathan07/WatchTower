...
RUN turbo prune cleaning-service --docker
...
COPY --from=installer --chown=nodeuser:nodejs /app/apps/cleaning-service/dist ./dist
COPY --from=installer --chown=nodeuser:nodejs /app/apps/cleaning-service/package.json .
